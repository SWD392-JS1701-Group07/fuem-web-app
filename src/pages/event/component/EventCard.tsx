...
const EventCard: React.FC<EventCardProps> = ({ event }) => {
  ...
  return (
    ...
    >
      <div className="flex h-full w-full cursor-pointer flex-col p-4">
        <img
        ...
        <h2 className="mt-4 text-2xl font-bold text-white">
          {event.name}
          <br />
          <span className="rounded-xl bg-indigo-400 px-4 text-xl font-semibold not-italic text-white">
        ...
        <h2 className="mt-1 italic text-gray-300">
          ...
          {[
            ...new Set(
              eventDetail?.scheduleList.map((schedule) => schedule.place || 'unknown location')
            )
          ].join(', ')}
        </h2>
        <p className="description my-2 flex-grow text-gray-300">
