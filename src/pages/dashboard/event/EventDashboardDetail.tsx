...
const EventDashboardDetail = () => {
  ...
  useEffect(() => {
    ...
    getAllSubject().then((res) => {
      ...
      setSubject(res.data.find((sub: Subject) => sub.id === event?.subjectId))
          ...
          <AccordionContent>
            <div className="absolute z-10 mt-4 flex space-x-2">
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-medium shadow-sm focus:outline-none focus:ring-indigo-500 focus:ring-offset-2"
              >
          ...
          <AccordionContent>
            {event?.scheduleList != null
              ? event?.scheduleList.map((schedule) => (
                  <Card className="pt-2 text-lg" key={schedule.id}>
